...
namespace sys {
	...
	class optional
	{
		static_assert(!sys::is_reference<T>::value, "cl::optional does not work with reference types");
		//static_assert(sys::is_destructible<T>::value, "cl::optional requires type to be Destructible");

	private:
		alignas(T) unsigned char stg_[sizeof(T)];  // We store any value here.
		...
		_CL_OPT_NORETURN void throw_bad_optional_access() const {
			FAIL sys::logic_error("bad_optional_access");
		}
		...
		//{
		//	new (to.pval_) T(sys::move(*from.pval_));
		//	from.pval_->~T();
		//	to.has_value_ = true;
	...
	public:
		...
		{
			if (has_value_) {
				new (pval_) T(sys::move(*obj.pval_));
			}
		}
		...
		// Swap implementation - Not needed.
	...
	_CL_OPT_CONSTEXPR auto make_optional(T&& val)
		-> optional<typename sys::decay<T>::type>
	{
		return optional<typename sys::decay<T>::type>(sys::forward<T>(val));
	}
