...
#include "System.h"
#include "tcsb.h"
template <typename ...args> extern int snprintf(char*, size_t, const char*, args...);
namespace sys {
	...
	static inline bool isnum(int c) {
		return is_char_integer(c);
	}
	static inline bool isalpha(int c) {
		return is_char_letter(c);
	}
		...
		while (*format && pos < size - 1) {
			if (*format == L'%') {
				...
				if (_is_it(format, L"ld")) {
					...
					wchar_t* decstr = _gdecstr(val_, 0);
					if (!decstr) decstr = L"~";
					wchar_t* itt = decstr;
					...
					wchar_t* hexstr = _ghexstr(val_, false, true);
					if (!hexstr) {
						hexstr = L"~";
					}
					wchar_t* itt = hexstr;
					...
					case L'f': { // originally not supported by system.
						double val_ = va_arg(args, double);
						wchar_t* decstr = _gdecstr(val_, count_spec);
						if (!decstr) decstr = L"~";
						wchar_t* itt = decstr;
						while (*itt && pos < size - 1) {
							buffer[pos++] = *itt++;
					...
					case L'X': {
						uint64_t val_ = va_arg(args, uint64_t);
						wchar_t* R = _ghexstr(val_, *format == L'X', false);
						if (R) {
							wchar_t* itt = R;
							while (*itt && pos < size - 1) {
								buffer[pos++] = *itt++;
			...
			if (*format == '%') {
				...
				if (_is_it(format, "ld")) {
					...
					case 'b': {
						...
						char* boolStr = val ? "true" : "false";
						for (int i = 0; boolStr[i] && pos < size - 1; ++i)
							buffer[pos++] = boolStr[i];
						break;
					}
					case 'd': {
						int value = va_arg(args, int);
