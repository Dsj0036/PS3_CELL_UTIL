...
void CStringBuilder::set_string_end() {
	_buffer[_cursor] = '\0';
}

/* reverse:  reverse string s in place */
void CStringBuilder::reverse(char s[])
{
	size_t i, j;
	char c;
...
void  CStringBuilder::raisecase(char s[])
{
	size_t i;
...
/**
 ...
 \endrst
 */
CStringBuilder::CStringBuilder(char* buffer, std::size_t bufferSize) {
	_buffer = buffer;
...
CStringBuilder::CStringBuilder(char(&array)[SIZE])
{
	_buffer = array;
	_bufferSize = SIZE;
	_cursor = 0;
...
template<int SIZE>
CStringBuilder& CStringBuilder::operator =(char(&data)[SIZE]) {
	_buffer = data;
	_bufferSize = SIZE;
	_cursor = 0;
	_isOverflow = false;
...
size_t  CStringBuilder::add_integer(IntType n)
{
...
size_t  CStringBuilder::buffer_size(void) { return _bufferSize; }


/** The actual size of the resulting string */
size_t  CStringBuilder::size(void) { return _cursor; }


/** returns pointer to a char[] buffer */
char* CStringBuilder::cstr() { return _buffer; }


/** Append by C string */
template<class CharConstPtr>
size_t  CStringBuilder::add(CharConstPtr array, std::size_t size) {
	if (_bufferSize - 1 == _cursor) return 0;
	size_t i;
...
/// add const string
template <std::size_t SIZE>
size_t  CStringBuilder::add(const char(&array)[SIZE]) {
	return add(&array[0], SIZE);
}

/// Adds char to the buffer
size_t  CStringBuilder::add(char value)
{
	if (_bufferSize - 1 == _cursor) return 0;
...
size_t  CStringBuilder::addb(bool value) {
	return add(value ? "true" : "false");
}
/// Adds hexadecimal literal to the buffer
size_t  CStringBuilder::addhex(uint32_t value) {
	char buff[14];
	_sys_snprintf(buff, 14, "  %x", value);
	raisecase(buff);
	buff[0] = '0';
	buff[1] = 'x';
	return add(buff);
...
/// Adds hexadecimal literal to the buffer
size_t  CStringBuilder::addhex(void* value) {
	return CStringBuilder::addhex((uint32_t)value);
}
size_t  CStringBuilder::add(bool value) { return add_integer<uint8_t>((uint8_t)value); }     /// Converts bool to string and adds to the buffer
size_t  CStringBuilder::add(int8_t value) { return add_integer<int8_t>(value); }     /// Converts int8_t to string and adds to the buffer
size_t  CStringBuilder::add(uint8_t value) { return add_integer<uint8_t>(value); }    /// Converts uint8_t to string and adds to the buffer
size_t  CStringBuilder::add(int16_t value) { return add_integer<int16_t>(value); }    /// Converts int16_t to string and adds to the buffer
size_t  CStringBuilder::add(uint16_t value) { return add_integer<uint16_t>(value); }   /// Converts uint16_t to string and adds to the buffer
size_t  CStringBuilder::add(int32_t value) { return add_integer<int32_t>(value); }    /// Converts int32_t to string and adds to the buffer
size_t  CStringBuilder::add(uint32_t value) { return add_integer<uint32_t>(value); }   /// Converts uint32_t to string and adds to the buffer
size_t  CStringBuilder::add(int64_t value) { return add_integer<int64_t>(value); }    /// Converts int64_t to string and adds to the buffer
size_t  CStringBuilder::add(uint64_t value) { return add_integer<uint64_t>(value); }   /// Converts uint64_t to string and adds to the buffer
...
const  TCSB_Fp& CStringBuilder::powers_ten(uint8_t index)
{
...
TCSB_Fp  CStringBuilder::find_cachedpow10(int exp, int* k)
{
...
const uint64_t* CStringBuilder::tens()
{
...
inline uint64_t  CStringBuilder::get_dbits(double d)
{
	union {
		double   dbl;
		uint64_t i;
...
TCSB_Fp  CStringBuilder::build_fp(double d)
{
...
void  CStringBuilder::normalize(TCSB_Fp* fp)
{
...
void  CStringBuilder::get_normalized_boundaries(TCSB_Fp* fp, TCSB_Fp* lower, TCSB_Fp* upper)
{
...
TCSB_Fp  CStringBuilder::multiply(TCSB_Fp* a, TCSB_Fp* b)
{
...
void  CStringBuilder::round_digit(char* digits, int ndigits, uint64_t delta, uint64_t rem, uint64_t kappa, uint64_t frac)
{
...
int  CStringBuilder::generate_digits(TCSB_Fp* fp, TCSB_Fp* upper, TCSB_Fp* lower, char* digits, int* K)
{
...
int  CStringBuilder::grisu2(double d, char* digits, int* K)
{
...
int CStringBuilder::emit_digits(char* digits, int ndigits, char* dest, int K, bool neg)
{
	int exp = TCDB_absv(K + ndigits - 1);

	/* write plain integer */
	if (K >= 0 && (exp < (ndigits + 7))) {
		_sys_memcpy(dest, digits, ndigits);
		_sys_memset(dest + ndigits, '0', K);
	...
	if (K < 0 && (K > -7 || exp < 4)) {
		...
		if (offset <= 0) {
			...
			dest[0] = '0';
			dest[1] = '.';
			_sys_memset(dest + 2, '0', offset);
			_sys_memcpy(dest + offset + 2, digits, ndigits);
			...
			_sys_memcpy(dest, digits, offset);
			dest[offset] = '.';
			_sys_memcpy(dest + offset + 1, digits + offset, ndigits - offset);
	...
	if (ndigits > 1) {
		dest[idx++] = '.';
		_sys_memcpy(dest + idx, digits + 1, ndigits - 1);
		idx += ndigits - 1;
	}
...
int  CStringBuilder::filter_special(double fp, char* dest)
{
	if (fp == 0.0) {
		dest[0] = '0';
		return 1;
...
size_t  CStringBuilder::fpconv_dtoa(double d, char dest[24])
{
	char digits[18];
...
size_t  CStringBuilder::addf(double value)
{
...
CharPointerHelper::CharPointerHelper(const char* buffer) : _buffer(buffer) {}

const char* CharPointerHelper::buffer() const { return _buffer; }
CharPointerHelper& CharPointerHelper::operator= (const char* s)
{
	this->_buffer = s;
	return *this;
}
